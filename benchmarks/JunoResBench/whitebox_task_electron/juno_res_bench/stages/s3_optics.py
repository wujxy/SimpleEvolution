...
def scint_weights(layout, vertex_m: np.ndarray, cfg: DetectorConfig) -> np.ndarray:
    ...
    """
    return scint_weights_steps(layout, np.asarray(vertex_m, np.float64).reshape(1, 3),
                               cfg.pmt_diameter_m)[0]
...
def _run_scint(photons, cfg, layout, rng, step_pos_m):
    scint = photons.photon_type == 0
    n_scint = int(scint.sum())
    if n_scint == 0:
        return None
    n_steps = len(step_pos_m)
    step_of_ph = photons.step_idx[scint]
    n_per_step = np.bincount(step_of_ph, minlength=n_steps)
    w = scint_weights_steps(layout, step_pos_m, cfg.pmt_diameter_m)  # (M, P)
    # per-step multinomial (single step = the legacy single call)
    pmt_idx = np.zeros(0, np.int32)
    for k in range(n_steps):
        nk = int(n_per_step[k])
        if nk == 0:
            continue
        n_arrived = rng.multinomial(nk, w[k])
        pmt_idx = np.concatenate(
            (pmt_idx, np.repeat(np.arange(layout.n_pmt, dtype=np.int32), n_arrived))
        )
    tofs = tof_ns_steps(layout.positions_m, step_pos_m, cfg.ls_refractive_index)
    tof_ph = tofs[step_of_ph, pmt_idx]
...
def _run_cherenkov(photons, cfg, layout, rng, step_pos_m):
    ...
    p = step_pos_m[photons.step_idx[cher]].astype(np.float64)   # (n,3) f64
    r_vtx = np.linalg.norm(p, axis=1)
    b = (d * p).sum(axis=1)
    disc = b * b + (layout.radius_m**2 - r_vtx**2)   # c2 = |pos|^2 - R^2 < 0
    t_path = -b + np.sqrt(np.maximum(disc, 0.0))     # pos -> hit point, m
    hit = p + t_path[:, None] * d
...
def run_s3(
    ...
    step_pos_m = (steps.pos_m.astype(np.float64) if steps is not None
                  else event.vertex_m.reshape(1, 3))
    # rng order note: scintillation draws first, Cherenkov second — enabling
    # Cherenkov never perturbs the scintillation stream.
    parts = [
        p
        for p in (
            _run_scint(photons, cfg, layout, rng, step_pos_m),
            _run_cherenkov(photons, cfg, layout, rng, step_pos_m),
        )
        if p is not None
    ]
    if not parts:
