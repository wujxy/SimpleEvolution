...
Scintillation branch (B1/B2/B5): per-step fluctuating photon count (Gauss
sqrt(N)), isotropic directions, 4-exponential emission times shifted by the
step deposition time (o-Ps-delayed light for annihilation steps).
Cherenkov branch (B4, default ON): per-step Frank-Tamm yield with the
electron beta from that step's deposited energy, photons on the cone
cos(theta_C) = 1/(n*beta) around the step's charged-particle direction,
prompt at the step time.

RNG draw order (electron events = the legacy sequence exactly):
  scint      normal(counts) -> uniform(ct) -> uniform(phi) -> choice -> exp
...
def _step_view(s1, event):
    """Per-step (e_vis, pos, t, e_dep, dir) arrays; stubs -> single step."""
    steps = getattr(s1, "steps", None)
    if steps is not None:
        return (steps.e_vis_mev, steps.pos_m, steps.t_ns,
                steps.e_dep_mev, steps.dir)
    ...
    return e_vis, pos, t, e_dep, d


def run_s2_scint(s1: S1Output, event, cfg: DetectorConfig, rng: np.random.Generator) -> PhotonSoA:
    e_vis, pos, t_step, _e_dep, _d = _step_view(s1, event)
    mu_gamma = e_vis * cfg.ly_photons_mev
    n_per_step = np.maximum(0.0, np.round(rng.normal(mu_gamma, np.sqrt(mu_gamma)))).astype(np.int64)
...
def run_s2_cherenkov(s1: S1Output, event, cfg: DetectorConfig, rng: np.random.Generator) -> PhotonSoA:
    ...
    _e_vis, pos, t_step, e_dep, d_step = _step_view(s1, event)
    beta = np.asarray([beta_from_kinetic(float(e)) for e in e_dep])
    cos_tc = 1.0 / (cfg.ls_refractive_index * beta)
    above = cos_tc < 1.0                 # per-step Cherenkov threshold
    ...
    lam = e_dep[idx] * cfg.ly_cherenkov * (1.0 - ct_k**2)
