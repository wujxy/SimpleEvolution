#!/usr/bin/env python3
"""Standalone online evaluator for positron visible-energy reconstruction."""
...
from scoring import score_positron
...
def run_online(submission, private_root, public_root):
    ...
    try:
        ...
        output = np.empty(len(split), dtype=float)
        for index, event in enumerate(split.iter_events()):
            ...
            output[index] = struct.unpack("!d", response)[0]
...
def main():
    ...
    with np.load(Path(args.private) / "truth.npz") as truth:
        role = truth["evt_sample_role"]
        result = score_positron(
            truth["evt_e_true"][role == 0], prediction[role == 0],
            truth["evt_e_vis"][role == 1], prediction[role == 1],
        )
