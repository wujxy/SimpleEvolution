...
class DetectorConfig:
    ...
    # ---- readout storage ----------------------------------------------------
    # False (frozen v1 contract): only channels with physics PEs get a
    # waveform row (their dark noise rides along); a random subset of up to
    # max_wf_per_event rows is stored. True (full readout, reality):
    # zero-suppressed DAQ — every channel with at least one in-window pulse
    # (physics PE or dark) is digitized; silent channels are omitted (their
    # absence is itself the measurement "quiet"), so the adc block is the
