...
R = 19.37 m). Each event is an equal mix of electron, gamma and positron events at an unknown position/energy/time.
Gammas deposit energy through a Compton chain
...
From the waveforms alone, reconstruct per event:
  - visible energy E_rec (MeV; for positrons this includes the annihilation light — the scored reference is
...
((q84-q16)/2 of t0_rec - t0_ref, ns). Reported overall and per particle type. Ranking: energy first, then
vertex, then timing.
